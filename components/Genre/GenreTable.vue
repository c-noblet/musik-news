<template>
  <div>
    <table class="rounded-t-lg w-full bg-gray-200 text-gray-800">
      ...
      <tbody>
        <tr v-for="genre in genres" :key="genre.id" :genre="genre" class="bg-gray-100 border-b border-gray-200">
          <td class="px-4 py-3">
            {{ genre.name }}
          </td>
          <td class="px-4 py-3">
            <div class="flex item-center justify-start">
              ...
              <button class="btn-action ml-5" @click="deleteGenre(genre.id)">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="#000000">
    ...
    <pagination :nb-element="nbGenres" :limit="limit" :current-page="currentPage" @changePage="changePage($event)" />
  </div>
</template>
